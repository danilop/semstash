...
class TestSemStashAgent:
    ...
    def test_agent_upload_then_new_agent_retrieves(
        ...
        # First agent uploads
        with create_agent(bucket=agent_bucket_name) as agent1:
        ...
        with create_agent(bucket=agent_bucket_name) as agent2:
            retrieve_result = get_response_text(
                agent2(f"Retrieve the content with key: {sample_text_file.name}")
            )
            # Agent should return file info (path, content type, etc.) or URL
            assert (
    ...
    def test_agent_gets_same_tools_as_mcp_server(self, agent_bucket_name: str) -> None:
        ...
        mcp_client = create_mcp_client(agent_bucket_name)
        with mcp_client:
        ...
        mcp_client2 = create_mcp_client(agent_bucket_name)
        with mcp_client2:
    ...
    def test_agent_upload_verified_in_semstash(
        ...
        # Agent uploads
        with create_agent(bucket=agent_bucket_name) as agent:
